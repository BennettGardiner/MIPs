...
# Bin pre-assignments
problem += v_bins[num_rows - 1][int(num_cols / 2) - 1] == 1
problem += v_bins[num_rows - 1][int(num_cols / 2)] == 1
...
if TABLES_TOUCH_DIAG:
    ...
    for row in range(num_rows - 1):
        for col in range(num_cols - 1):
            problem += v_tables[row][col] + v_tables[row + 1][col] + v_tables[row][col + 1] + v_tables[row + 1][
                col + 1] <= 1
...
for row in range(1, num_rows - 1):
    for col in range(1, num_cols - 1):
        problem += v_chairs[row][col] <= v_tables[row + 1][col] + v_tables[row - 1][col] + v_tables[row][col + 1] + \
                   v_tables[row][col - 1]
...
for col in range(1, num_cols - 1):
    problem += v_chairs[num_rows - 1][col] <= v_tables[num_rows - 2][col] + v_tables[num_rows - 1][col + 1] + \
               v_tables[num_rows - 1][col - 1]
for row in range(1, num_rows - 1):
    problem += v_chairs[row][0] <= v_tables[row + 1][0] + v_tables[row - 1][0] + v_tables[row][1]
for row in range(1, num_rows - 1):
    problem += v_chairs[row][num_cols - 1] <= v_tables[row + 1][col] + v_tables[row - 1][num_cols - 1] + v_tables[row][
        num_cols - 2]
...
problem += v_chairs[num_rows - 1][num_cols - 1] <= v_tables[num_rows - 2][num_cols - 1] + v_tables[num_rows - 1][
    num_cols - 2]
